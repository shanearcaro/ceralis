...
function newTable(questionNum, text, questionId){
    ...
    submitButton.addEventListener("click", function() {updateTable(questionNum, questionId)});
    ...
    ajax.onreadystatechange = function() {
        if (ajax.readyState == 4 && ajax.status == 200){
            ...
            ajax.onloadend = function() {
                updateTotalScore(studentExamId);
...
function updateTotalScore(studentExamId){
    const requestCode = 34;
    const credentials = `studentexamid=${studentExamId}&request=${requestCode}`;
    var totalEarned = 0;
    var totalWorth = 0;
    var total;
    ...
    ajax.onreadystatechange = function() {
        if (ajax.readyState == 4 && ajax.status == 200){
            ...
            total = totalEarned/totalWorth * 100;

            finalUpdate(total, studentExamId);
        }
    }
    ajax.open("POST", "https://afsaccess4.njit.edu/~sma237/CS490/controller/request-model.php", true);
    ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
function finalUpdate(total, studentExamId){
    const requestCode = 35;
    const credentials = `studentexamid=${studentExamId}&score=${total}&request=${requestCode}`;
    const ajax = new XMLHttpRequest();
    ajax.onreadystatechange = function() {
        if (ajax.readyState == 4 && ajax.status == 200){
            //The response text will be the id of the last exam created
